...
function AuthShell({ title, subtitle, features, children }) {
  return (
    <main className="dark auth-grid-pattern relative flex min-h-screen items-center justify-center overflow-hidden bg-[#0f172a] px-4 py-6 sm:px-6 lg:px-10">
            ...
            <CardContent className="flex h-full flex-col justify-between p-8 xl:p-10">
              ...
              <div className="animate-slide-in-left">
                <div className="mb-10 flex items-center gap-3">
                  ...
                  <span className="text-xl font-semibold tracking-tight text-foreground dark:text-white/90">EmPay</span>
                </div>
              ...
              <div className="mt-auto space-y-2.5 pt-8">
                <Separator className="mb-4 bg-border dark:bg-white/[0.06]" />
                <p className="mb-3 text-[0.65rem] font-semibold tracking-[0.2em] text-muted-foreground/60 dark:text-white/30 uppercase">
                  What you get
                </p>
                {features.map((feature, i) => (
                  <div
                    key={feature.label}
                    className={`feature-pill animate-fade-up delay-${i + 3} flex items-center gap-3 rounded-xl border border-border/60 dark:border-white/[0.07] bg-muted/40 dark:bg-white/[0.03] px-4 py-3`}
                  >
                    ...
                    <span className="text-sm font-medium text-foreground dark:text-white/80">{feature.label}</span>
                  </div>
                ))}
              </div>
            </CardContent>
